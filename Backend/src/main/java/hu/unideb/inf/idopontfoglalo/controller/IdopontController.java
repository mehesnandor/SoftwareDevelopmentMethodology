...
public class IdopontController {
    ...
    public List<FoglaltIdopontok> getFoglaltIdopontokByEquelDate(@RequestParam LocalDateTime date_filter) {
        log.info("[Controller] - Listázzása (" + date_filter + ") -nek");
        return idopontService.getFoglaltIdopontokByEquelDate(date_filter);
    }
    ...
    public void deleteByEquleDate(@RequestParam("date") LocalDateTime date) {
        log.info("[Controller] - Törlése az (" + date + ") dátummal rendelkező foglalásnak");
        idopontService.deleteByEquleDate(date);
    }
